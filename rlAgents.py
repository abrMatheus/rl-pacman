...
class AQLAgent(Agent):
    ...
    def featArray(self, action, dist):
        ...
        if (dist['ghost']<2): features[1] = 1.0                                 #2-um so fantasma
        features[2] = dist['ghost']                                             #3-ok
        features[3] = 1/dist['ghost']                                           #4-ok
        
        features[4] = dist['edible_ghost']                                      #5-ok
        if (dist['edible_ghost']<=2): features[5] = 1.0                         #6-um so fantasma
        features[6] = dist['capsules']                                          #7-ok
        
        if( dist['ghost']<3 and dist['capsules']<2): features[7] = 1.0          #9 - booleana
        if (dist['ghost']>3 and dist['food']<3) : features[8] = 1.0             #10 - ok
        if (self.last_a == self.actions[taken_action_index]): features[9] = 1.0 #11 - eu add
        features[-1] = 1                                                        #bias
    ...
    def egreedyAction(self, state):
        ...
        self.epslon = self.i_epslon/np.exp(self.num_actions*0.25)
        return action, score